...
adsl_care_long <- read_csv("/Users/tma/Desktop/advancedR/input_df.csv")
...
###Race groups
```{r p_race, out.width = "100%"}
#race group-----------------------------------------------------------------
d_race <- adsl_care_long %>%
      dplyr::filter(pt_chars == "racecat", value != "blank") %>%
      dplyr::add_count(pt_chars, network, icdat_rev2) %>%
      dplyr::add_count(pt_chars, network, icdat_rev2, value ) %>%
      dplyr::distinct(icdat_rev2, network, pt_chars, network, value, n, nn ) %>%
      dplyr::mutate(pct = nn / n)
...
###Medical history (most relavent ones)
```{r p_mh, out.width = "100%"}
...
d_mh_top <- left_join(cnt, d_mh, by="pt_chars") %>%
      ...
      dplyr::distinct(icdat_rev2, network, pt_chars, var_lbl, rank, network, value, n, nn ) %>%
